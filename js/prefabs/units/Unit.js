...
RPG.Unit = function (game_state, name, position, properties) {
    ...
    this.DAMAGE_TEXT_STYLE = {font: "bold 24px Kells", fill: "#FF0000"};
    this.DAMAGE_TEXT_DURATION = 1;
...
RPG.Unit.prototype.receive_damage = function (damage) {
    ...
    damage_text = this.game_state.game.add.text(this.x, this.y - 50, "" + damage, this.DAMAGE_TEXT_STYLE, this.game_state.groups.hud);
    kill_timer = this.game_state.time.create();
    kill_timer.add(this.DAMAGE_TEXT_DURATION * Phaser.Timer.SECOND, damage_text.kill, damage_text);
    kill_timer.start();
};
